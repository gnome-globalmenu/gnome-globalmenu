...
typedef struct {
	gboolean disposed;
	GdkWindow * obj_win;
	GList * connections; /*ConnInfo*/
	GQueue return_stack; /*ReturnInfo*/
	guint dead_connection_detection;
...
static gchar * _query_objects 	( GnomenuService * service, const gchar * name, gchar * args);
static gchar * _touch			( GnomenuService * service, const gchar * name, gchar * args);

static gboolean _dead_connection_detect(GnomenuService * service);
static GdkFilterReturn
	_obj_win_filter (GdkXEvent* gdkxevent, GdkEvent * event, GnomenuService * service);
static GdkFilterReturn
	_conn_win_filter (GdkXEvent* gdkxevent, GdkEvent * event, GnomenuService * service);
...
gnomenu_service_class_init(GnomenuServiceClass * klass){
	...
	gnomenu_service_class_install_method(klass, 
			"QueryObjects", _query_objects, NULL);
}
static void
gnomenu_service_init(GnomenuService * service) {
	...
	self->object_table = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, g_object_unref);
...
*/
static gchar * _invoke_parse(GnomenuService * service, gchar * command){
	gchar ** splitted = g_strsplit_set(command, " ", 4);
	gchar * rt = _invoke(service, splitted[1], splitted[2], splitted[3]);
	g_strfreev(splitted);
	return rt;
}
static gchar * _invoke(GnomenuService * service, const char * object, const gchar * name, gchar * args){
...
static ConnInfo * _find_conn_from_native(GnomenuService * service, GdkNativeWindow window){
	GET_OBJECT(service, self, priv);
	GList * node;
	for(node = priv->connections; node; node=node->next){
		if(GDK_WINDOW_XWINDOW(((ConnInfo*)(node->data))->conn_win) == window)
			return node->data;
	}
	return NULL;
}
...
static void _method_return(GnomenuService * service, GnomenuXMessage msg, const gchar * rt){
	...
	if(rt == NULL) /*a void method*/ {
		...
		_send_xclient_message(/*GDK_WINDOW_XWINDOW(pi->conn_win)*/ msg.source, &msg_r, sizeof(msg_r));
	} else {
		ConnInfo * ci = _find_conn_from_native(self, msg.source);
		ReturnInfo * ri = g_new0(ReturnInfo, 1);
		ri->id = msg.id;
		ri->ret_loop = g_main_loop_new(NULL, TRUE);
		ri->conn_win = ci->conn_win;
		ri->done = FALSE;
		ri->buffer = rt;
		...
		_send_xclient_message(/*GDK_WINDOW_XWINDOW(ci->conn_win)*/ msg.source, &msg_r, sizeof(msg_r));
	}
			...
			if(xevent->xproperty.state == PropertyNewValue){
				...
				if( x_prop_name_decode(property_name, &buffer_name, &id)
				 && g_str_equal(buffer_name, _GNOMENU_METHOD_RETURN)){
					...
					g_queue_for(&priv->return_stack, ReturnInfo * ri, 
						if(ri->conn_win == event->any.window 
...
void gnomenu_service_register(GnomenuService * service, GnomenuObject * object){
	GET_OBJECT(service, self, priv);
	LOG("register object %s", object->name);
	g_hash_table_insert(self->object_table, object->name, g_object_ref(object));
}

void gnomenu_service_dispose(GnomenuService * service, const gchar * name){
	GET_OBJECT(service, self, priv);
	g_hash_table_remove(self->object_table, name);
}
GnomenuObject * gnomenu_service_steal(GnomenuService * service, const gchar * name){
	GnomenuObject * rt;
	GET_OBJECT(service, self, priv);
	rt = g_hash_table_lookup(self->object_table, name);
	g_hash_table_steal(self->object_table, name);
	return rt;
}
...
void gnomenu_service_expose(GnomenuService * service){
	...
	priv->dead_connection_detection = g_timeout_add_seconds(5, _dead_connection_detect, self);
}
static gboolean _dead_connection_detect(GnomenuService * service){
	GET_OBJECT(service, self, priv);
	GList * node;
	GList * q;
	for(node = priv->connections; node; node=q){
		ConnInfo * ci = node->data;
		q = node->next;
		if(ci->idle_time > 2) {
			/*TODO: ensure the target is no long a connection to me, can detect the window name*/
			LOG("one connection is dead, clean it up");
			ci->dead = TRUE;
			priv->connections = g_list_remove_link(priv->connections, node);
			g_list_free1(node);
		} else
			ci->idle_time ++;
	}
...
static gchar * _accept ( GnomenuService * service, const gchar * name, gchar * args){
	GET_OBJECT(service, self, priv);
	ConnInfo * ci = g_new0(ConnInfo, 1);
	LOG("arg = %s", args);
	GdkNativeWindow w;
	sscanf(args, "%d", &w);
	ci->conn_win = gdk_window_foreign_new(w);
	ci->idle_time = 0;
	ci->dead = FALSE;
	priv->connections = g_list_append(priv->connections, ci);
	g_assert(ci->conn_win);
	LOG("binded by a new proxy at %d", w);
	gdk_window_set_events(ci->conn_win, gdk_window_get_events(ci->conn_win) |
				GDK_PROPERTY_CHANGE_MASK);
	gdk_window_add_filter(ci->conn_win, _conn_win_filter, service);
	return NULL;
}
...
static gchar * _query_objects 	( GnomenuService * service, const gchar * name, gchar * args){
	...
	GList * list = g_hash_table_get_values(service->object_table);
	GList * node;
	gint i;
	buffer = g_new0(gpointer, g_list_length(list)+1);
	for(node = list, i=0; node; node = node->next, i++){
		GnomenuObject * obj = node->data;
		LOG("object:%s ", obj->name);
		buffer[i] = g_strdup_printf("%s %s", obj->name, G_OBJECT_TYPE_NAME(obj));
	}
	buffer[i] = NULL;
	rt = g_strjoinv("\n", buffer);
	g_list_free(list);
...
static gchar * _touch			( GnomenuService * service, const gchar * name, gchar * args){
	LOG("touched by a connection, args = %s", args);
	return NULL;
}
